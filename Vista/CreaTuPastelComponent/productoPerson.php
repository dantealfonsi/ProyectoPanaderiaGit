        ...
        if(isset($_SESSION['carrito_compras'])){
            ...
            $ids_producto = array_column($_SESSION['carrito_compras'], 'id');
                ...
                if(!in_array($_GET['id_producto'], $ids_producto)){//** */
                    ...
                    $Q_insertar_en_cartitem = 'INSERT INTO itemcarrito (iscustom,idproducto, idcarrito, precio, cantidad, motivo) 
VALUES (1,'.$_SESSION['IDproducto'].','.$_SESSION['IDcarrito'].','.filter_input(INPUT_POST, 'precio').','.filter_input(INPUT_POST, 'cantidad_entrada').', "'.filter_input(INPUT_POST, 'motivo').'" )';
                    $ejecutar_insertar_en_cartitem = mysqli_query($conn, $Q_insertar_en_cartitem) or exit("Error: " . mysqli_error($conn));
                }
            ...
            $Q_insertar_en_cartitem = 'INSERT INTO itemcarrito (iscustom,idproducto, idcarrito, precio, cantidad,motivo) 
            VALUES (1,'.$_GET['id_producto'].','.$_SESSION['IDcarrito'].','.filter_input(INPUT_POST, 'precio').','.filter_input(INPUT_POST, 'cantidad_entrada').',"'.filter_input(INPUT_POST, 'motivo').'")';
            $ejecutar_insertar_en_cartitem = mysqli_query($conn, $Q_insertar_en_cartitem) or exit("second:" .mysqli_connect_error());
        }
    }
}
...
<div class="container mx-auto mt-0 pt-0 ">
    ...
    <div class="row">
        ...
        <div class="col mt-4">
            ...
            <!-- INPUT CANTIDAD -->
            <form id="form-pd" method="POST" action="productoPerson.php?action=add&id_producto=<?php echo $id_producto; ?>">
                <div class="box my-4">
