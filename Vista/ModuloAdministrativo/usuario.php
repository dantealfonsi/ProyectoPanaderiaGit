...
<?php
include '../../Controlador/gestionar_usuario.php';
$usuario = new Usuario;   
$tmodulo=new Modulo;
...
if (isset($_GET['detalle'])){
  ...
  <section style='display: flex; justify-content: space-between;'>
      ...
      <div>
      <h1 class='titulo-Subtitulo'>DATOS DE USUARIO</h1>
      <h1 class='subtitle_container'><span></span>$titulo</h1>
      </div>

      <a href='usuario.php' class='close-btn'> ⌦ </a> 
  ...
  $consulta = "select * from usuario WHERE deleted=0";    
        ...
        buttons:[

            {
                ...
                buttons: [
                    {
                      extend:    'excelHtml5',
                      text:      '<img id="table_icon" src="../../Assets/images/inventory/excel.svg">> EXCEL</i>',
...
let restrictNombreUsuario = false;
let restrictEditNombreUsuario = true;

let restrictCedulaUsuario = false;
let restrictEditCedulaUsuario = true;

let restrictPassword = false;
let restrictEditPassword = true;

let restrictNivel = false;
let restrictEditNivel = true;
...
function valCedula(){
  let campo = document.getElementById('cedulaUsuario');

      $("#cedulaUsuario").css("border-bottom","2px solid #6acfff");
      ...
      datos= JSON.parse(data);
        if(datos.existe==='0'){
          $("#cedulaUsuario").css("border-bottom","2px solid #f27474");
          restrictCedulaUsuario = false;
          restrictEditCedulaUsuario=false;
          Swal.fire(
                'Advertencia',
                'Esta cedula no pertenece a un empleado de la empresa',
                'warning'
              ).then(function(){
                campo.value='';
              });
        ...
        $.post("../modelo/modulo_proyecto.php",{
          ...
          Swal.fire(
                'Advertencia',
                'Esta cedula ya Pertenece a un Usuario',
                'warning'
              ).then(function(){
                campo.value='';
                campo.focus();
        ...
        $("#cedulaUsuario").css("border-bottom","2px solid #6acfff");
        restrictCedulaUsuario = true;
        restrictEditCedulaUsuario=true;
      }
        });
      }
...
let tempNombre = document.getElementById("nombreUsuario").value;
  
  function valNombreUsuario(){
    let campo=document.getElementById("nombreUsuario");
    if(document.getElementById("nombreUsuario").value.length===0){

      $("#nombreUsuario").css("border-bottom","2px solid #f27474");

      tippy('#nombreUsuario', {
          content: "Introduce un nombre de Usuario",
          trigger: 'focus',
          animation: 'fade',
        });
            ...
            datos= JSON.parse(data);
              ...
              if(datos.existe==='1' && campo.value != tempNombre ){
                $("#nombreUsuario").css("border-bottom","2px solid #f27474");
              ...
              restrictNombreUsuario = false;
              restrictEditNombreUsuario = false;
            } 
            else {
            $("#nombreUsuario").css("border-bottom","2px solid #6acfff");
            restrictNombreUsuario = true; 
            restrictEditNombreUsuario = false;        
          }
      });
    }
...
function valPassword(){
      let pattern=/.{8}/;
      let campo = document.getElementById('passwordUsuario');
     
      let result = pattern.test(campo.value);
      restrictPassword = result;
      restrictEditPassword = result;
      if(result === false){
        $("#passwordUsuario").css("border-bottom","2px solid #f27474");

        tippy('#password', {
          content: "Introduzca una contraseña de 8 caracteres!",
          trigger: 'focus',
          animation: 'fade',
        });
      }
      else{
        $("#passwordUsuario").css("border-bottom","2px solid #6acfff");
      }
  }
  ...
  function valNivel(){
      let pattern=/^[01]$/;
      let campo = document.getElementById('nivelUsuario'); 
      let result = pattern.test(campo.value);
      restrictNivel = result;
      restrictEditNivel = result;
      if(result === false){
        $("#nivelUsuario").css("border-bottom","2px solid #f27474");

        tippy('#nivel', {
          content: "Solo puedes tener 2 niveles!",
          trigger: 'focus',
          animation: 'fade',
        });
      }
      else{
        $("#nivelUsuario").css("border-bottom","2px solid #6acfff");
      }
  }
...
if(restrictCedulaUsuario===true && restrictNombreUsuario===true && restrictPassword===true && restrictNivel===true){
  restrict=true;
}else{
  restrict=false;
}

if (restrict===true && document.getElementById("nombreUsuario").value.length>0 && document.getElementById("cedulaUsuario").value.length>0 && document.getElementById("nivelUsuario").value.length>0 && document.getElementById("passwordUsuario").value.length>0){
...
function guardarNuevo(idx) {

if(restrictEditCedulaUsuario===true &&
  restrictEditNombreUsuario===true && 
  restrictEditPassword===true && 
  restrictEditNivel===true){
  restrictEdit=true;
}else{
  restrictEdit=false;
}
...
if (restrictEdit===true && 
  ...
  document.getElementById("cedulaUsuario").value.length>0 && 
  document.getElementById("nivelUsuario").value.length>0 && 
  document.getElementById("passwordUsuario").value.length>0){
